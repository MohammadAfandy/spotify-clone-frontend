...
const CardCollection: React.FC<CardCollectionProps> = ({
  ...
  const bgColor = 'bg-gradient-to-b ' + (type === 'track' ? 'from-green-300 to-green-800' : 'from-blue-300 to-blue-800');
  return (
    <div
    ...
    >
      <div className="text-justify mb-4 text-sm">
        {type === 'track' && tracks && (tracks.map((track, idx) => (
          <Fragment key={idx}>
        ...
        {type === 'episode' && episodes && (episodes.map((episode, idx) => (
          <Fragment key={idx}>
            <span className="font-bold">
              {episode.name}
              &nbsp;
