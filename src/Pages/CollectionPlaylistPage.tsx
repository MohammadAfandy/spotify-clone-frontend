...
const CollectionPlaylistPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCollectionPlaylist = async () => {
      ...
      setPlaylistTracks(dataPlaylistTrack.data.items.map((item: { added_at: Date, track: Track }) => ({
        ...item.track,
  ...
  const handlePlayEpisode = async () => {
    const response = await ApiSpotify.get('/me/episodes', { params: { limit: 50 } });
    const episodeUris = response.data.items.map((v: { added_at: Date, episode: Episode }) => v.episode.uri);
